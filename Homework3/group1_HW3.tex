...
\item Dunder Mifflin
 ...
 \begin{tikzpicture}[rotate=270,transform shape,
      ...
      \node[mycircle] (v0){$v_0$};
      \node[mycircle,above right=of v0] (g){$g$};
      \node[mycircle,below right=of v0] (s){$s$};
	...
	\node[mycircle, above =of pgst] (pgu){$pg_u$};
	  ...
	  \node[mycircle, right =of sc] (isc){$I_{sc}$};
      \node[mycircle, below =of isc] (iu){$I_u$};
      \node[mycircle, below =of iu] (ist){$I_{st}$};

	  \node[mycircle, below right =of isc] (tg){$T_{g}$};
      \node[mycircle, below =of tg] (tfb){$T_{fb}$};

	  \node[mycircle, right =of tg] (dg){$D_{g}$};
      \node[mycircle, below =of dg] (dfb){$D_{fb}$};
...
\draw[myarrow,red] (sc) to [out = -45, in=-135] node[sloped, font = \tiny, above] {$(135,0,100)$} (isc);
\draw[myarrow,red] (u) to [out = -45, in=-135] node[sloped, font = \tiny, above] {$(157.5,0,100)$} (iu);
\draw[myarrow,red] (st) to [out = -45, in=-135] node[sloped, font = \tiny, above] {$(172.5,0,100)$} (ist);
